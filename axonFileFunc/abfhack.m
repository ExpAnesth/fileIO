function [si,h]=abfhack_v1p2(fn,varargin)
% ** function [si,h]=abfhack(fn,varargin)
% loads and returns data in ABF (Axon Binary File) format.
% modified 8Aug2012 to create backup of data before filtering - MIB
% modified 13Aug2012, including input arguments for artifact removal - HH
%
% *************************************************************************
% *************************************************************************
% * ABFHACK, BASED ON ABFLOAD, FILTERS AND REWRITES DATA INTO ABF FILES. BY
% * NECESSITY, INPUT ARGUMENTS START, STOP, SWEEPS AND CHANNELS ARE IGNORED
% * AND THE WHOLE DATA WILL BE FILTERED. THE FOLLOWING NEW INPUT ARGUMENTS
%   ARE IMPLEMENTED (NOTE UPPER/lower CASE):
%   cFreq - ARRAY OF FREQUENCIES FOR A NOTCH FILTER (HZ, DEFAULT [60])
%   afTsl - ARRAY OF TIME STAMPS OF ARTIFACTS TO BE BLANKED (MS)
%   afDur - DURATION OF ARTIFACT (MS)
%   blankVal - VALUE TO WHICH BLANKED PORTION OF DATA SHALL BE SET
%   (DEFAULT=0)
% * CURRENTLY (NOV 2012) FILTERING AND REWRITING IS ONLY IMPLEMENTED FOR
% * FIXED-LENGTH AND GAP-FREE DATA BUT CAN EASILY BE MODIFIED FOR OTHER
% PURPOSES. NOTE THAT ARTIFACT REMOVAL WILL NOT BE PERFORMED ON GAP-DATA.
% *************************************************************************
% *************************************************************************
%
...
%         [d,si,h]=abfhack('d:\data01.abf','info') 
% abfhack will not load any data but return detailed information (header
% parameters) on the file in output variable h. d and si will be empty.
% In all other cases abfhack will load data. Optional input parameters
% listed below (= all except the file name) must be specified as
% parameter/value pairs, e.g. as in 
%         d=abfhack('d:\data01.abf','start',100,'stop','e');
%
% >>> INPUT VARIABLES >>>
...
%                                 to 'a', all channels will be read. NOTE:
%                                 channel order in output variable d
...
%                                 be read by abfhack 'ieee-be' is the 
%                                 alternative.
% << OUTPUT VARIABLES <<<
...
%   Date of this version: May 7, 2012

% BUG FIXES
% + May 2012:
% The order of channels in input variable 'channel' is ignored by abfhack;
% instead, data is always put out according to an order inherent to the abf
% file (to be retrieved in header parameter h). In the previous version of
% abfhack, specifying an order different from the inherent channel order
% could result in wrong scaling of the data (if the scaling differed
% between channels). This is fixed.
...
chunk=0.05;
machineF='ieee-le';
% * ABFHACK-SPECIFIC INPUT ARGUMENTS
cFreq=60;
afTsl=[];
afDur=[];
blankVal=0;
...
% -------------------------------------------------------------------------
suffixPos = strfind(fn,'.abf');
% new file name
fn_temp = [fn(1:suffixPos(1)-1) '_filt.abf'];
% copy original file using this name
[status] = copyfile(fn,fn_temp);
if ~status
  error('Failed to copy raw data file');
end
% abfhack now uses this file name
fn = fn_temp;
...
switch h.nOperationMode
  case 1
    ...
    if h.fFileVersionNumber>=2.0
      errordlg('abfhack currently does not work with data acquired in event-driven variable-length mode and ABF version 2.0','ABF version issue');
    else
  ...
  case {2,4,5}
    ...
    if doLoadData
      for i=1:nSweeps,
        ...
        % *****************************************************************
        % * ARTIFACT BLANKING (ON ALL SWEEPS; ASSUMPTION IS THAT AF DOES
        % NOT SHIFT ACROSS SWEEPS)
        tmpd=tmpd';
        % *** blank stim artifacts
        if ~isempty(afTsl) && ~isempty(afDur)
          % index to data to be plotted: [-1 1] s around first stim artifact 
          plotIx=round((afTsl(1)+[-1000 1000])*(1000/h.si));
          plotIx=[max(plotIx(1),1) min(plotIx(2),h.dataPtsPerChan)];
          if diff(plotIx)>0
            % pick first channel
            plotD=tmpd(plotIx(1):plotIx(2),1);
          end
          % loop over artifacts
          for afi=1:numel(afTsl)
            % generate index to first and last data point to be blanked
            blankIx=round((afTsl(afi)+[0 afDur])*(1000/h.si));
            if blankIx(1)<1 || blankIx(2)>h.dataPtsPerChan
              warning(['artifact #' int2str(afi) ' could not be blanked because the blanking interval is at least partly outside the recorded data']);
            else
              tmpd(blankIx(1):blankIx(2),:)=blankVal;
            end
          end
        else
          % index to data to be plotted, default: first 2 s
          plotIx=round([1 2000]*(1000/h.si));
          plotIx=[max(plotIx(1),1) min(plotIx(2),h.dataPtsPerChan)];
          plotD=tmpd(plotIx(1):plotIx(2),1);
        end
        % * FILTERING
        for ffi=1:numel(cFreq)
          tmpd=elimhum(tmpd,h.si,cFreq(ffi));
        end
        % * SHOW EXCERPTS
        if diff(plotIx)>0
          figure(111), clf
          plotD=cat(2,plotD,tmpd(plotIx(1):plotIx(2),1));
          plot(plotD)
          axis tight
          title([fn ', sweep ' int2str(i) ' (excerpt)'],'interpreter','none')
          % this is a hint that if data are in integer format they are not
          % scaled here, and that if the blanking value is to be different
          % from zero, it will have to be specified in the native units
          ylabel('native unit')
        ...
        tmpd=tmpd';
        fseek(fid,selectedSegStartInPts(i),'bof');
        n2=fwrite(fid,tmpd,precision);
        if n2~=dataPtsPerSweep,
          fclose(fid);
          error(['something went wrong WRITING episode ' int2str(sweeps(i)) ': ' dataPtsPerSweep ' points should have been written, ' int2str(n2) ' points actually written']);
        end
        % *****************************************************************
  ...
  case 3
    ...
    if doLoadData
      ...
      elseif length(chInd)/h.nADCNumChannels<1
        ...
        % * FILTERING
        for ffi=1:numel(cFreq)
          d=elimhum(d,h.si,cFreq(ffi));
        end
        % * WRITING BACK DATA
        d=d';
        fseek(fid,startPt*dataSz+headOffset,'bof');
        n2=fwrite(fid,d,precision);
        if n2~=h.dataPts,
          fclose(fid);
          error(['something went wrong WRITING file (' int2str(h.dataPts) ' points should have been read, ' int2str(n) ' points actually read']);
        end
        % *****************************************************************
...
rp=0.5;			% max attenuation in passband, don't change
rs=70;			% attenuation in dB per octave
